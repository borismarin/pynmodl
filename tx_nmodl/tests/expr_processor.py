...
class NoTypes(object):
    def __init__(self):
        self.mm = metamodel_from_str('''
            ...
            Primed: variable=VarRef+"'" '='!'='? expression=LogicalCon;
    ...
    def varref(self, var):
        ...
        if(self.is_txtype(var.var, 'FuncPar')):
            ...
            self.stack2.append('{}_{{{}}}_{{{}}}'.format(var.var.name, *parnames)) 
        else:
    ...
    def exp(self, exp):
        self.op(exp)
    def funccall(self, f):
        args = self.unpack_stack(f.args)
        self.stack.append('{}({})'.format(f.func.name, ', '.join(args)))
        
        args = self.unpack_stack2(f.args)
        self.zzz.append(f.func.template.format(**dict(zip([p.name for p in f.func.pars], args))))
        self.stack2.append('{}({})'.format(f.func.name, ', '.join(args)))
    def assign(self, asgn):
        if asgn.variable :
            ...
            r = self.stack2.pop()
            l = self.stack2.pop()
            self.stack2.append('<DerivedVariable name="{}" value="{}/>"'.format(l,r))
        else:
            exp = self.stack.pop()
    ...
    def primed(self, p):
        ...
        var = self.stack2.pop()
        self.zzz.append('<TimeDerivative variable="{}" value="{}"/>'.format(var, expression))
        
        
    def unpack_stack(self, node):
        return reversed([self.stack.pop() for _ in node])
    
    def unpack_stack2(self, node):
        return reversed([self.stack2.pop() for _ in node])
...
def main(debug=False):
    ...
    print(c.gen_nml("G V  FUNCTION f(x,y){LOCAL a a=2*x f=a+y} x' = 1 + f(3,V) y'=2+f(4,V)"))
